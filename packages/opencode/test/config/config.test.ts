...
const expandToken = (text: string) => text.replaceAll("{env:TEST_TOKEN}", "test-token")

function wellKnownAuth(input: {
  authUrl?: string
  config?: unknown
  ...
  return Layer.mock(AuthWellKnown.Service)({
    ...
    configs: () =>
      Effect.gen(function* () {
        ...
        if (metadata && typeof metadata === "object" && "remote_config" in metadata && metadata.remote_config) {
          const remoteConfig = metadata.remote_config as { url: string; headers?: Record<string, string> }
          const remoteUrl = expandToken(remoteConfig.url)
          input.seen.remote = remoteUrl
          if (remoteConfig.headers?.Authorization) {
            input.seen.authorization = expandToken(remoteConfig.headers.Authorization)
          }
          const remote = input.remote
          const content =
            remote && typeof remote === "object" && "config" in remote && remote.config && typeof remote.config === "object"
              ? remote.config
              : remote
          docs.push({
            url: remoteUrl,
